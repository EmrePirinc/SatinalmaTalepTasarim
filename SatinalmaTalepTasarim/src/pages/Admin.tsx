...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userResponse = await fetch(`http://localhost:3001/api/auth/me/${userId}`)
        if (!userResponse.ok) {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/auth/users', {
        ...
        body: JSON.stringify({
          ...
          name: formData.name,
          role: formData.role,
          purchaseAuthority: formData.purchasingAuth,
          financeAuthority: formData.financeAuth,
        }),
      })
  ...
  const handleEditUser = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/auth/users/${selectedUser.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        alert(data.error || "Kullanıcı güncellenirken hata oluştu!")
        return
      }
                          ...
                          try {
                            const response = await fetch(`http://localhost:3001/api/auth/users/${user.id}`, {
                              ...
                              body: JSON.stringify({
                                name: user.name,
                                role: user.role,
                                purchaseAuthority: user.purchasingAuth,
                                financeAuthority: user.financeAuth,
                                status: newStatus,
                              }),
                            })
